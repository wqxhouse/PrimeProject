...
void *MemoryManager::nextAlligned(void *ptr)
{
	...
	unsigned int modAliign = (unsigned int)ptr % ALLIGNMENT;
	if (modAliign)
	{
		// ptr is not ALLIGNMENT alligned
		allignedPtr = (void *)((unsigned int)allignedPtr + ALLIGNMENT - modAliign);
	}
	return allignedPtr;
}
void MemoryManager::Construct()
{
	...
	PEINFO("Memory manager allocated memory %dB at 0%x\n", totalMemoryNeeded + ALLIGNMENT, (int)(ptr));
	...
	for (unsigned int i = 0; i < N_MEMORY_POOLS; i++)
	{
		...
		allignedPtr = (void *)((unsigned int)(allignedPtr) + poolSize);
