...
namespace ATG
{
	...
	VOID NuiJointConverter::CalculateJointRotations()
	{
		...
		XMVECTOR hipDir = XMVectorSubtract(  m_NuiSkeleton.SkeletonPositions[ NUI_SKELETON_POSITION_HAND_RIGHT ], 
			m_NuiSkeleton.SkeletonPositions[ NUI_SKELETON_POSITION_HAND_LEFT ] ); 
		hipDir.y = 0;
	...
	INT NuiJointConverter::MapAvatarJointToIntIndex(DWORD avatarJointIndex)
	{
		INT intIndex = -1;
		for ( INT i = 0; i < ARRAYSIZE( g_IntermediateToAvatarMap ); ++i )
		{
			if (g_IntermediateToAvatarMap[i].m_avatarJoint== avatarJointIndex)
			{
				intIndex = g_IntermediateToAvatarMap[i].m_intJoint;
				break;
			}
		}
		return intIndex;
	}
	INT NuiJointConverter::MapAvatarJointToNUI_POSITION_INDEX(DWORD avatarJointIndex)
	{
		INT intIndex = -1;
		for ( INT i = 0; i < ARRAYSIZE( g_IntermediateToAvatarMap ); ++i )
		{
		...
		if (intIndex != -1)
		{
			return g_IntJointDesc[intIndex].startNuiJoint;
		}
		else
		{
			return -1;
	...
	XMVECTOR NuiJointConverter::GetAvatarJointWorldPosition(DWORD avatarJointIndex)
	{
		INT intIndex = -1;
		for ( INT i = 0; i < ARRAYSIZE( g_IntermediateToAvatarMap ); ++i )
		{
		...
		if (intIndex != -1)
		{
			NUI_SKELETON_POSITION_INDEX nuiJointIndex = g_IntJointDesc[intIndex].startNuiJoint;
			XMVECTOR vJointPos = m_vReconstructedPos[nuiJointIndex];//GetNuiSkeletonJointPos( nuiJointIndex );
	...
	XMVECTOR NuiJointConverter::GetNUI_POSITION_INDEXWorldPosition(NUI_SKELETON_POSITION_INDEX index)
	{
			XMVECTOR vJointPos = m_vReconstructedPos[index];//GetNuiSkeletonJointPos( nuiJointIndex );
			return vJointPos;
	}
